...
import { Timer, MapPin, Leaf, Package, Bike, Car, AlertCircle, TrendingUp, Wind, Thermometer, Cloud, Droplets } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  const actions = [
    { icon: Package, label: 'Order Food', color: 'text-orange-400', info: '15-20 min delivery', route: '/dashboard/quick-actions/order-food' },
    { icon: Bike, label: 'Book Transport', color: 'text-green-400', info: '3 bikes nearby', route: '/dashboard/quick-actions/book-transport' },
    { icon: Car, label: 'Car Service', color: 'text-blue-400', info: 'Valet available', route: '/dashboard/quick-actions/car-service' },
    { icon: AlertCircle, label: 'Emergency', color: 'text-red-400', info: 'SOS ready', route: '/dashboard/quick-actions/emergency' },
  ]
    ...
    <>
      ...
      <div className="bg-purple-500/20 border border-purple-500/30 rounded-xl p-4 mb-8">
        <div className="flex items-center space-x-3">
          ...
          <p className="text-purple-200">Traffic is expected to improve by 40% in the next 30 minutes</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div className="flex justify-between items-center">
              {trafficTrends.map((trend, index) => (
                <div key={trend.time} className="flex flex-col items-center">
                  ...
                  <span className="text-xs text-zinc-500 mt-2">{trend.time}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="bg-zinc-900 rounded-xl p-6 border border-zinc-800">
          ...
          <div className="mt-4">
            ...
            <p className="text-sm text-green-400">75% towards weekly goal</p>
          </div>
        </div>
      ...
      <div className="mt-6 bg-zinc-900 rounded-xl p-6 border border-zinc-800">
        ...
        <div className="space-y-4">
          {[
            { time: '2m ago', event: 'Route optimization suggested', type: 'info' },
            { time: '15m ago', event: 'Weather alert: Light rain expected', type: 'warning' },
            { time: '1h ago', event: 'Completed eco-friendly trip', type: 'success' },
          ].map((activity, index) => (
            <div key={index} className="flex items-center space-x-4 p-3 rounded-lg bg-zinc-800/50">
